...
export default function HomeNavbar() {
  ...
  const NewProductAdded = useSelector((state) => state.Cart.NewProductAdded);

  const dispatch = useDispatch();
  const navigate = useNavigate();

  const token = JSON.parse(localStorage.getItem("rentifyToken")) || null;
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:8000/user/getname", {
        headers: {
          token,
        },
      })
  ...
  useEffect(() => {
    const filteredData = data.filter((el) => el.title.includes(searchValue));
    setFilterData(searchValue ? filteredData : []);
  }, [searchValue, data]);
        ...
        >
          ...
          <Flex flex={{ base: 1 }} justify={{ base: "center", md: "start" }}>
            <Text textAlign={useBreakpointValue({ base: "center", md: "left" })} fontFamily="heading" color={useColorModeValue("gray.800", "white")}>
              <RefLink to="/">
                <Box display="flex" gap="10px" alignItems="center" cursor="pointer">
          ...
          <Stack flex={{ base: 1, md: 0 }} justify="flex-end" direction="row" spacing={{ sm: "0", base: "0", md: "6", lg: "6", xl: "6" }}>
            {isLogin ? (
              <Button as="a" fontSize="sm" fontWeight={400} variant="link" display="flex" alignItems="center" gap="5px" width={{ base: "100px", sm: "100px" }}>
                <span>
                  <IoCartOutline />
                ...
                <div style={displayDiv} onMouseEnter={handleDisplay} onMouseLeave={removeDisplay} className="hover-div">
                  ...
                  <RefLink to="/cart">
                    <Stack>
                      <button style={{ backgroundColor: "red", color: "white", padding: "10px 0px", fontSize: "14px", fontWeight: "bold" }}>
                        Take me to cart
                      </button>
                    </Stack>
                  </RefLink>
...
const DesktopNav = ({ handleChange, filterData, searchValue, setSearchValue }) => {
  return (
    <Stack direction="row" spacing={4}>
      <VStack>
        <SearchBar handleChange={handleChange} filterData={filterData} searchValue={searchValue} setSearchValue={setSearchValue} />
      </VStack>
    </Stack>
  );
};
